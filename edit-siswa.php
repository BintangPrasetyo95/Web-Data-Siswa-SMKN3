...
if (isset($_POST['nama_siswa'])) {
	...
	if ($nama_siswa == "" || $nis == "") {
		...
		$query = mysqli_query($connect, "UPDATE siswa SET nis='$nis',nama_siswa='$nama_siswa',tanggal_lahir='$tanggal_lahir',jk='$jk',alamat='$alamat',id_kelas='$id_kelas',no_telp='$no_telp' WHERE id_siswa='$id' ");
		}
	}
...
<?php  
$query = mysqli_query($connect, "SELECT * FROM siswa INNER JOIN kelas ON siswa.id_kelas=kelas.id_kelas INNER JOIN jurusan ON kelas.id_jurusan=jurusan.id_jurusan WHERE id_siswa='$id' ");
					...
					<div class="mb-3">
						...
						<div class="col-sm-12">
							<select name="id_kelas" class="form-select">
								<?php
								$query2 = mysqli_query($connect, "SELECT * FROM kelas INNER JOIN jurusan ON kelas.id_jurusan=jurusan.id_jurusan");
								while ($data2 = mysqli_fetch_array($query2)) {
								?>								
								<option value="<?php echo $data2['id_kelas']; ?>" <?php if ($data['id_kelas'] == $data2['id_kelas']) {echo "selected";} ?>>
								<?php echo $data2['nama_kelas']; ?> - <?php echo $data2['nama_jurusan']; ?></option>
								<?php
								}
								?>
							</select>
